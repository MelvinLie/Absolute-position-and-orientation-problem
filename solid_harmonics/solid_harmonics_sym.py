...
Here, axis symmety is exploited.
'''
def compute_Br_mat_sym(r,theta,phi,L):

    M = len(r)

    ret_mat = np.zeros((M,L))

    for l in range(1,L+1):

        ret_mat[:,l-1] = l*r**(l-1)*np.real(sph_harm(0,l,phi,theta))
...
Here, axis symmety is exploited.
'''
def compute_Bt_mat_sym(r,theta,phi,L):
    ...
    for l in range(1,L+1):
        ...
        ret_mat[:,l-1] = r**(l-1)*np.real(dYdt(0,l,phi,theta))
...
Here, axis symmety is exploited.
'''
def compute_meas_mat_sym(p,s,n,L):
    M = p.shape[0]
    ...
    Br_mat = compute_Br_mat_sym(r,t,p,L)
    Bt_mat = compute_Bt_mat_sym(r,t,p,L)
    ...
    return s[0]*(n[0]*Bx_mat + n[1]*By_mat + n[2]*Bz_mat) + s[1]

'''
Evaluate Hall probe measurements, based on real solid  harmonic coefficients.
Here, axis symmety is exploited.
'''
def compute_meas_sym(p,s,n,c):
...
Here, axis symmety is exploited.
'''
def M_sym(r,c,ay,az,bx,bz,gx,gy,dx,dy,dz,sx,sy,sz):
    ...
    #number of Dofs for field
    L = len(c)
    #total number of DoFs with latent variables
    DoFs = L + 15
    ...
    y[:M] = compute_meas_sym(rx,sx[0],nx,c) + sx[1]
    ...
    #compute measurements
    y[M:2*M] = compute_meas_sym(ry,sy[0],ny,c) + sy[1]
    ...
    #compute measurements
    y[2*M:] = compute_meas_sym(rz,sz[0],nz,c) + sz[1]
...
J = Jac_M(...)[:,:K+6]
'''
def Jac_M_sym(r,c,ay,az,bx,bz,gx,gy,dx,dy,dz,sx,sy,sz):
    ...
    Br_mat_x = compute_Br_mat_sym(Rx,Tx,Px,L)
    Bt_mat_x = compute_Bt_mat_sym(Rx,Tx,Px,L)
    ...
    Br_mat_y = compute_Br_mat_sym(Ry,Ty,Py,L)
    Bt_mat_y = compute_Bt_mat_sym(Ry,Ty,Py,L)
    ...
    Br_mat_z = compute_Br_mat_sym(Rz,Tz,Pz,L)
    Bt_mat_z = compute_Bt_mat_sym(Rz,Tz,Pz,L)
    ...
    #***************************************************
    #probe x:
    J[:num_meas,:L] = sx[0]*(nx[0]*Bx_mat_x + nx[1]*Bx_mat_y + nx[2]*Bx_mat_z)

    #probe y:
    J[num_meas:2*num_meas,:L] = sy[0]*(ny[0]*By_mat_x + ny[1]*By_mat_y + ny[2]*By_mat_z)

    #probe z:
    J[2*num_meas:,:L] = sz[0]*(nz[0]*Bz_mat_x + nz[1]*Bz_mat_y + nz[2]*Bz_mat_z)

    #alpha_y
    J[num_meas:2*num_meas,L] = sy[0]*(ny_day[0]*By_x \
                   + ny_day[1]*By_y \
                   + ny_day[2]*By_z )

    #alpha_z
    J[2*num_meas:,L+1] = sz[0]*(nz_daz[0]*Bz_x \
                   + nz_daz[1]*Bz_y \
                   + nz_daz[2]*Bz_z )

    #beta_x
    J[:num_meas,L+2]    = sx[0]*(nx_dbx[0]*Bx_x \
                   + nx_dbx[1]*Bx_y \
                   + nx_dbx[2]*Bx_z )

    #beta_z
    J[2*num_meas:,L+3]  = sz[0]*(nz_dbz[0]*Bz_x \
                   + nz_dbz[2]*Bz_z )

    #gamma x
    J[:num_meas,L+4]    = sx[0]*(nx_dgx[0]*Bx_x \
                   + nx_dgx[1]*Bx_y )

    #gamma y
    J[num_meas:2*num_meas,L+5] = sy[0]*(ny_dgy[0]*By_x \
                   + ny_dgy[1]*By_y )


    #for numerical approximation
    y_0 = M_sym(r,c,ay,az,bx,bz,gx,gy,dx,dy,dz,sx,sy,sz)

    #d_x^x
    tmp_d = dx.copy()
    tmp_d[0] += eps_d
    J[:,L+6] = (M_sym(r,c,ay,az,bx,bz,gx,gy,tmp_d,dy,dz,sx,sy,sz) - y_0) / eps_d

    #d_x^y
    tmp_d = dx.copy()
    tmp_d[1] += eps_d
    J[:,L+7] = (M_sym(r,c,ay,az,bx,bz,gx,gy,tmp_d,dy,dz,sx,sy,sz) - y_0) / eps_d

    #d_x^z
    tmp_d = dx.copy()
    tmp_d[2] += eps_d
    J[:,L+8] = (M_sym(r,c,ay,az,bx,bz,gx,gy,tmp_d,dy,dz,sx,sy,sz) - y_0) / eps_d

    #d_y^x
    tmp_d = dy.copy()
    tmp_d[0] += eps_d
    J[:,L+9] = (M_sym(r,c,ay,az,bx,bz,gx,gy,dx,tmp_d,dz,sx,sy,sz) - y_0) / eps_d

    #d_y^y
    tmp_d = dy.copy()
    tmp_d[1] += eps_d
    J[:,L+10] = (M_sym(r,c,ay,az,bx,bz,gx,gy,dx,tmp_d,dz,sx,sy,sz) - y_0) / eps_d

    #d_y^z
    tmp_d = dy.copy()
    tmp_d[2] += eps_d
    J[:,L+11] = (M_sym(r,c,ay,az,bx,bz,gx,gy,dx,tmp_d,dz,sx,sy,sz) - y_0) / eps_d

    #d_z^x
    tmp_d = dz.copy()
    tmp_d[0] += eps_d
    J[:,L+12] = (M_sym(r,c,ay,az,bx,bz,gx,gy,dx,dy,tmp_d,sx,sy,sz) - y_0) / eps_d

    #d_z^y
    tmp_d = dz.copy()
    tmp_d[1] += eps_d
    J[:,L+13] = (M_sym(r,c,ay,az,bx,bz,gx,gy,dx,dy,tmp_d,sx,sy,sz) - y_0) / eps_d

    #d_z^z
    tmp_d = dz.copy()
    tmp_d[2] += eps_d
    J[:,L+14] = (M_sym(r,c,ay,az,bx,bz,gx,gy,dx,dy,tmp_d,sx,sy,sz) - y_0) / eps_d

    #Sensitivities:
    J[:num_meas,L+15]           = (nx[0]*Bx_x + nx[1]*Bx_y + nx[2]*Bx_z)
    J[num_meas:2*num_meas,L+16] = (ny[0]*By_x + ny[1]*By_y + ny[2]*By_z)
    J[2*num_meas:,L+17]         = (nz[0]*Bz_x + nz[1]*Bz_y + nz[2]*Bz_z)
